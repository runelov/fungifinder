...
// Ingen Turnstile her — i motsetning til /auth/be-om-lenke (som tar imot en
// vilkårlig oppgitt e-post) krever denne ruten at man allerede har en
...
export async function sjekkInvitasjon({ request, env, params }) {
  ...
  const rad = await env.DB.prepare(
    'SELECT epost FROM invitasjoner WHERE token_hash = ?1 AND brukt = 0 AND utloper > ?2'
  )
    .bind(hash, Date.now())
    .first();
...
export async function registrerMedInvitasjon({ request, env, params }) {
  ...
  // krever samme 256-bit engangsbevis som et magic-link-klikk, så dette ER
  // en aktivering — ikke bare en registrering.
  let bruker;
  try {
